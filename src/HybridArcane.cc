...
Init(IParallelMng* iPMng)
{
  ...
  int local_size = iPMng->commSize() / iMPMng->commSize();
  int local_rank = iPMng->commRank() % iMPMng->commSize();
  ...
  if (local_rank == 0) {
    m_requests = new UniqueArray<Request>[local_size];
    m_iPMng = new UniqueArray<Ref<IParallelMng>>[local_size];
    m_isInit = true;
  }
  ...
  m_requests[local_rank].resize(1);
  m_requests[local_rank][MPA_Request_null].reset();

  // Le premier IParallelMng est le IParallelMng global passé en paramètre.
  m_iPMng[local_rank].resize(1);
  m_iPMng[local_rank][MPA_COMM_WORLD] = makeRef(iPMng);

  // On enregistre la correspondance entre l'id du thread et son rang.
  iPMng->threadMng()->beginCriticalSection();
  m_tids[std::this_thread::get_id()] = local_rank;
...
Comm_split(MPA_Comm comm, int color, int key, MPA_Comm* newcomm)
{
  ...
  // On commence par déterminer le min.
