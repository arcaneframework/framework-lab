...
initModule()
{
  ...
  pos_fis = options()->csvOutput()->getPosRow("Nb de Fissions");
  pos_col = options()->csvOutput()->getPosRow("Nb de Collisions");
...
loopModule()
{
  ...
  // Ici, on utilise editElemDown() pour ajouter un élement sous l'élement dernièrement
  // modifié.
  options()->csvOutput()->addElemRow(pos_fis, nb_fissions);
  options()->csvOutput()->editElemDown(nb_fissions/2.);
  options()->csvOutput()->addElemRow(pos_col, nb_collisions);
  options()->csvOutput()->editElemDown(nb_collisions/2.);
...
endModule()
{
  ...
  for(Integer pos = 0; pos < options()->csvOutput()->getNumRows(); pos++) {
    RealUniqueArray row = options()->csvOutput()->getRow(pos);
    Real sum = 0.;
    for(Real elem : row) {
      sum += elem;
    }
    options()->csvOutput()->addElemColumn("Somme", sum);
  }
