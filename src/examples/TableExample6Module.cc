...
initModule()
{
  ...
  options()->csvOutput()->editElem(0, 0, 1);
...
loopModule()
{
  ...
  // vers la dernière case modifiée. On peut aussi demander à getElem() 
  ...
  Real elem1 = options()->csvOutput()->getElem(-1, m_global_iteration()-1, true);

  // On récupère la valeur au début de la ligne et on met à jour le pointeur.
  Real elem2 = options()->csvOutput()->getElem(0, m_global_iteration()-1, true);
  ...
  for(Integer i = 0; i < m_global_iteration()+1; i++){
    ...
    options()->csvOutput()->editElemDown(elem1 + elem2, false);
    elem1 = elem2;
    // On récupère l'élement à droite du pointeur et on force la mise à jour du pointeur.
    elem2 = options()->csvOutput()->getElemRight(true);
  }
